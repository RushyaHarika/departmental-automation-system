...
app.put("/api/certification/:id",async(req,res)=>{
    const updatedCertification=await Certification.updateOne({ _id:req.params.id },{ $set: {courseName:req.body.courseName, score:req.body.score, issuedBy:req.body.issuedBy, certificate:req.body.certificate, topper:req.body.topper, year:req.body.year, cycle:req.body.cycle} });
...
/*Coordinator*/
const Coordinator=mongoose.model("coordinator",new mongoose.Schema({
    _id:{type:String, default:shortid.generate},
    cid:{
        type:String,
        required:"Coordinator Id is required",
        unique:true
    },
    name:String,
    email:String,
    type:{
        type:String,
        required:"Coordinator Type is required",
        unique:true
    }
}))

/** Coordinator Data */
app.get("/api/coordinator",async (req,res)=>{
    const coordinator=await Coordinator.find({});
    res.send(coordinator);
});

app.get("/api/coordinator/:email",async (req,res)=>{
...
app.post("/api/coordinator",async (req,res)=>{
    ...
    .then((response) =>{
       ...
       .catch( (error)=> {
        let err="";
        if(error.code===11000){
            err="Coordinator id and type must be unique";  
        }
        if(error.errors!==undefined){
            if(error.errors.cid!==undefined){
                err=error.errors.cid.properties.message;
            }else if(error.errors.type!==undefined){
                err=error.errors.type.properties.message;
            }
        } 
        return res.status(400).json({
            "error": err
...
app.delete("/api/coordinator/:id",async(req,res)=>{
    const deletedCoordinator=await Coordinator.deleteOne({cid:req.params.id});
    res.send(deletedCoordinator);
})

app.put("/api/coordinator/:id",async(req,res)=>{
    const updatedCoordinator=await Coordinator.updateOne({ cid: req.params.id },{ $set: {name:req.body.name, mobile:req.body.mobile, qualification:req.body.qualification, designation:req.body.designation} });
    res.send(updatedCoordinator);
})

/*Coordinator*/
const WorkshopAttended=mongoose.model("workshopAttended",new mongoose.Schema({
    _id:{type:String, default:shortid.generate},
    sNo:String,
    regNo:String,
    stuName:String,
...
/** Coordinator Data */
app.get("/api/workshopAttended",async (req,res)=>{
    const coordinator=await WorkshopAttended.find({});
    res.send(coordinator);
});

app.post("/api/workshopAttended",async (req,res)=>{
    const newCoordinator=new WorkshopAttended(req.body);
    await newCoordinator.save()
    .then((response) =>{
        console.log(response);
        res.send(response);
...
app.delete("/api/workshopAttended/:id",async(req,res)=>{
    const deletedCoordinator=await WorkshopAttended.deleteOne({_id:req.params.id});
    res.send(deletedCoordinator);
})

app.put("/api/workshopAttended/:id",async(req,res)=>{
    const updatedCoordinator=await WorkshopAttended.updateOne({ _id: req.params.id },{ $set: {sNo:req.body.sNO, regNo:req.body.regNo, stuName:req.body.stuName, type:req.body.type,name:req.body.name,from:req.body.from,to:req.body.to} });
    res.send(updatedCoordinator);
})
