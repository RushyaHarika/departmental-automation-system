...
function JournalEditModal(props) {
    ...
    const EditJournal =async (id)=>{
        const res=await fetch("/api/journal/"+id,{
            ...
            body:JSON.stringify({
              title:(title?title:props.editItem.title), author:(author?author:props.editItem.author), authPos:(authPos?authPos:props.editItem.authPos), nameOfJournal:(nameOfJournal?nameOfJournal:props.editItem.nameOfJournal), impact:(impact?impact:props.editItem.impact), volume:(volume?volume:props.editItem.volume), issue:(issue?issue:props.editItem.issue), page:(page?page:props.editItem.page), date:(date?date:props.editItem.date), type:(type?type:props.editItem.type)
            })
          });
    ...
    return (
          ...
          <form method="POST"> 
              ...
              <input className='col-sm-6' type='textbox' defaultValue={props.editItem.nameOfJournal} onChange={ e => setNameOfJournal(e.target.value)}/>
    ...
    <Modal.Footer>
        <Button variant='primary' type='submit' style={{float:'right'}} onClick={() => { EditJournal(props.editItem._id) }}>Submit</Button>
    </Modal.Footer>
  </Modal>
    );
  }
