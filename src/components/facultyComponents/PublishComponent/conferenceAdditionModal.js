...
function ConferenceAdditionModal(props) {
    ...
    const PostConference =async ()=>{
      ...
      const res=await fetch("/api/conference",{
        ...
        body:JSON.stringify({
          fid, title, author, authPos, nameOfConference, impact, volume, issue, page, date, type
        })
      });
      ...
      if(res.status===400){
        ...
        window.alert("Conference added successfully");
        setTitle('');
        setDate('');
        setAuthor('');
        setAuthPos('');
      ...
      >
        ...
        <Modal.Body>
          <form method="POST"> 
              ...
              <input className='col-sm-6' type='textbox' value={nameOfConference} onChange={ e => setNameOfConference(e.target.value)}/>
  ...
  export default ConferenceAdditionModal;